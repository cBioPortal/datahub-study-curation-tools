...
and CopyNumberAlterationUtilDao.getHugoToEntrezMap.

The Java merge util throws on conflicting event codes, but its caller catches
every exception and falls back to the original file, which the downstream
importer (ImportTabDelimData) dedupes first-row-wins per resolved gene. This
script reproduces both paths:
  - file merges cleanly -> fused rows written (status MERGED)
  - any conflict or Entrez parse error -> whole file falls back to
    first-row-wins dedup (status FALLBACK)

See README.md for the gene table inputs and the known deviations from the Java
implementation.
