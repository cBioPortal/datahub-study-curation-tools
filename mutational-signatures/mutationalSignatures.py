...
"""
Mutational signatures wrapper script. Create mutation matrices and extract mutational
signatures from mutational data using tempoSig. For usage, run the script with the -h option.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Extract mutational signatures (using tempoSig) from a VCF/MAF/cBioPortal study directory and "
                    "generate output in cBioPortal format.",
    )
    ...
    input_excl.add_argument("--study-path", metavar="./path/", dest="study_path", type=check_path,
                            help="cBioPortal study path")
    input_excl.add_argument("--maf", metavar="file.maf", type=check_path,
                            help="MAF file (--study-id required)")
    input_excl.add_argument("--vcf-folder", metavar="./path/", dest="vcf_folder", type=check_path,
                            help="folder containing VCF files with sampleID as filename "
                                 "(--study-id and --genome required)")
    # REQUIRED ARGUMENTS
    required_group = parser.add_argument_group("required arguments")
    required_group.add_argument("-t", "--temposig-loc", dest="temposig_loc", metavar="temposig.R", type=check_path,
    ...
    required_group.add_argument("-o", "--out-dir", metavar="./path/", dest="out_dir", required=True,
                                type=lambda x: check_path(x, is_dir=True), help="output directory")
    # SIGNATURE FILES
    ...
    sigs.add_argument("-s", "--cosmic-sbs-file", dest="sbs_file", metavar="cosmic_SBS.txt",
                      type=check_path, help="extract single base substitution signatures with the provided file")
    sigs.add_argument("-d", "--cosmic-dbs-file", dest="dbs_file", metavar="cosmic_DBS.txt",
                      type=check_path, help="extract double base substitution signatures with the provided file")
    sigs.add_argument("-i", "--cosmic-id-file", dest="id_file", metavar="cosmic_ID.txt",
                      type=check_path, help="extract insertion/deletion signatures with the provided file")
    ...
    optional_group.add_argument("--tmp-dir", dest="tmp_dir", metavar="./path/", default="./tmp",
                                type=lambda x: check_path(x, is_dir=True, empty_path=True),
    ...
    optional_group.add_argument("--annotate", metavar="annotate.csv", type=check_path,  # Location relative to repo
                                default=osp.normpath(osp.join(osp.dirname(__file__), 
                                                     "mutational_signatures_annotation.csv")),
                                help="Path to signature annotation file (default: "
                                     "mutational_signatures_annotation.csv)")
    optional_group.add_argument("--mask", metavar="0.05", type=float, default=0.05,
                                help="Add metafile mask for contributions below given value (default 0.05)")
...
def check_path(path, is_dir=False, empty_path=False):
    """Check if file or directory exists
    
    :param path: file or directory path 
    :type path: str
    :param is_dir: path should be dir, create if non-existent
    :param empty_path: raise error if path exists
...
def implemented_genome(genome: str) -> str:
    """Check if genome has been implemented.
...
def arg_conditionals(args: argparse.Namespace):
    ...
    if args.maf or args.study_path:  # Warning for MAF
        print("!! When using MAF/cBioPortal studies as input, be aware that your files might be pre-processed and the "
              "mutations might not represent the true mutational landscape. This can influence the output of "
...
def read_mutations_meta_file(study_path: str) -> dict:
    ...
    for file in meta_files:
        yaml_cont = load_yaml(file)
        if yaml_cont.get("genetic_alteration_type") == "MUTATION_EXTENDED" and yaml_cont.get("datatype") == "MAF":
...
def find_maf_study_id(study_path: str) -> tuple:
...
def determine_alternative_allele(maf: pd.DataFrame) -> str:
    ...
    Follows cBioPortal documentation: https://docs.cbioportal.org/file-formats/#tumor-seq-allele-ambiguity
...
def preprocess_maf(mut_file: str, tmp_dir: str, args: argparse.Namespace) -> tuple:
    ...
    Remap MAF file using only the important parts, as the cBioPortal MAF format does not conform to
    ...
    # SigProfilerMatrixGenerator needs the following columns at these specific locations
    for i, j in enumerate([4, 5, 6, 10, 12, 15]):  
        new_headers[j] = headers[i]
    # Re-order the maf file so the columns are in the correct position
...
def copy_vcf(args: argparse.Namespace, tmp_dir):
...
def prepare_input(args: argparse.Namespace) -> tuple:
    ...
    if args.study_path:
        ...
        maf_file, study_id = find_maf_study_id(args.study_path)
    elif args.maf:
        maf_file = args.maf
    else:
        maf_file = None
...
def matrix_stable_id(string: str) -> str:
    ...
    return "mutational_signatures_matrix_" + string
...
def matrix_to_cbioportal(matrix: str, outfile: str):
    ...
    id_cols = ["ENTITY_STABLE_ID", "NAME"]
    data = data[id_cols + data.columns.difference(id_cols).tolist()]
...
def temposig_to_generic_assay(infile: str, outfile:str, annotation_file: str, data_type: str) -> str:
    ...
    data = data.drop(columns="Number of Mutations")
    data = data.fillna("NA")
...
def calculate_mutational_signatures(tmp_dir: str, out_dir: str, args: argparse.Namespace) -> list:
    ...
    for substitution_matrix, matrix_type, cosmic_file in [("output/SBS/tmp.SBS96.all", "SBS", args.sbs_file),
        ...
        elif not osp.exists(inf_matrix):
...
def generate_single_meta_file(study_id: str, meta_file: str, file_type: str, algorithm_string: str, out_dir: str,
                              mask: str=0.0, meta_properties: str="NAME,DESCRIPTION,URL", nperm: str=None):
    ...
    :param out_dir: path to output directory
    :param mask: masking value for small numbers
    :param meta_properties: generic_entity_meta_properties string (default: 'NAME,DESCRIPTION,URL')
    :param nperm: number of permutations for pvalue estimation
    """
    ...
    out_lines = f"cancer_study_identifier: {study_id}\n" \
                ...
                f"{algorithm_string if file_type != 'matrix' else 'SigProfilerMatrixGenerator'}" \
                f"{' with ' + nperm + ' permutations' if nperm and file_type != 'matrix' else ''}\n" \
                ...
                f"value_sort_order: DESC"
    if file_type == "contribution":
        out_lines += f"\npivot_threshold_value: {mask}"
        ...
        for matrix in matrices_ran:
            ...
            if file_type == "matrix":
                generate_single_meta_file(study_id, meta_filename, file_type, algorithm_string, args.out_dir, 
                                          meta_properties="NAME", nperm=str(args.nperm))
            else:
                generate_single_meta_file(study_id, meta_filename, file_type, algorithm_string, args.out_dir,
                                          mask=args.mask, nperm=str(args.nperm))
