...
def map_fusion_to_sv(fusion_dict):
	...
	for id in fusion_dict:
		fusion_column_value = fusion_dict[id][len(fusion_dict[id])-1]['Event_Info']
		...
		for row in range(len(fusion_dict[id])):
			...
			fusion_dict[id][row]['Event_Info'] = fusion_column_value
...
def main(parsed_args):
	...
	for row in data:
		...
		# From event info column extract the geneA and geneB
		geneA = row['Hugo_Symbol']
		if 'archer' not in row['Event_Info'].lower():
			geneB = list(filter(None, row['Event_Info'].split(geneA)))[0].strip('-').strip(' ').split(' ',1)[0]
			forward_event = sample_id+'_'+geneA+'-'+geneB
			reverse_event = sample_id+'_'+geneB+'-'+geneA
		else:
			geneB = list(filter(None, row['Event_Info'].split(geneA)))[0].strip('-').strip(' ').split(' ',1)[0]
			forward_event = sample_id+'_'+geneA+'-'+geneB+'_Archer'
			reverse_event = sample_id+'_'+geneB+'-'+geneA+'_Archer'
		
		if forward_event not in fusion_dict and reverse_event not in fusion_dict:
			fusion_dict[forward_event] = list()
			fusion_dict[forward_event].append(row)
		elif forward_event in fusion_dict:
			fusion_dict[forward_event].append(row)
		elif reverse_event in fusion_dict:
			fusion_dict[reverse_event].append(row)
