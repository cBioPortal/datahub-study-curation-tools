...
def write_data(final_mapped_data, header, outfile):
	...
	additional_columns = [x for x in header if x not in updated_header]
	additional_columns = [x for x in additional_columns if x not in ['index', 'Hugo_Symbol', 'Entrez_Gene_Id', 'Tumor_Sample_Barcode']]
...
def map_fusion_to_sv(fusion_dict):
	...
	for id in fusion_dict:
		fusion_column_value = fusion_dict[id][0]['Fusion']
		...
		#Group by index values. (Picks the first occurence if duplicated)
		dict1 = {}
...
def file_format_check(header, infile):
	...
	for element in min_required_columns:
		if element not in header:
			print("The input fusion file: "+infile+" format is invalid. Please refer to https://docs.cbioportal.org/5.1-data-loading/data-loading/file-formats#fusion-data for the allowed fields.")
			sys.exit(1)
...
def interface():
	...
	parser.add_argument('-repo', '--source_repo', nargs='?', const='cmo-argos', type=str, default='cmo-argos', help = 'Defaulted to cmo-argos. Enter the source repo of the fusion file otherwise.')
...
def main(parsed_args):
	...
	file_format_check(header, parsed_args.fusion_file)
	
	fusion_dict = {}
	for row in data:
		key_val = row['Tumor_Sample_Barcode']+'_'+row['Fusion']
	...
	if parsed_args.source_repo == 'cmo-argos':
		if 'Frame' in header: header = ['Connection_Type' if item == 'Frame' else item for item in header]
		for val in sv_data:
			if 'Frame' in val:
				val['Connection_Type'] = val.pop('Frame')

	write_data(sv_data, header, parsed_args.sv_file)
