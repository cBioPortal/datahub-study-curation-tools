...
Fuses extra `data_mutations*.txt` files into the study's primary MAF (what the
pipelines JAR effectively does by importing all pattern-matched MAFs into one
profile), in a metaImport-compatible way: columns remapped by header name,
first-wins dedup on the validator's duplicate-mutation key, secondary data +
meta files removed. `--exclude <filename>` leaves a secondary alone (e.g. an
uncalled MAF meant to stay a separate profile).
